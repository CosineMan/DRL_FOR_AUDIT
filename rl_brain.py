...
class RL(object):
    ...
    def choose_action(self, observation, episode):
        ...
        if np.random.uniform() < self.epsilon: #當貪婪發生
            ...
            # print('亂數前的', state_action) # 開發驗證用，正式執行時註解起來
            state_action = state_action.reindex(np.random.permutation(state_action.index))  # 當值相同時用random重排前後，再用idxmax挑出最先出現的，已實現隨機性
            # print('亂數後的', state_action) # 開發驗證用，正式執行時註解起來
            action = state_action.idxmax()  #選擇Q表中最大值
            # print(f'在{observation}狀態時') # 開發驗證用，正式執行時註解起來
            # print(f'選擇走{action}') # 開發驗證用，正式執行時註解起來
    ...
    def check_state_exist(self, state):
        # 開發時檢查資料格式，正式運作時可註解
        # print(f'state的值:{state}')
        # print(f'state型態:{type(state)}')
