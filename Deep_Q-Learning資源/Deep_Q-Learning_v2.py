...
epsilon_decay = (epsilon - epsilon_min) / 1000000 # 探索衰減率
update_target_every = 10000  # 目標網路更新頻率
...
# 讀取預訓練權重
load_pre_train = True
model_weight_name = 'dqn_for_space_invaders_v5_v20250317.keras'

# 切換工作路徑
try:
    os.chdir('./Deep_Q-Learning資源')
    print('工作路徑切換為./Deep_Q-Learning資源')
except OSError:
...
def build_model():
    ...
    x = keras.layers.BatchNormalization()(x)  # 穩定梯度
    ...
    x = keras.layers.Dropout(0.2)(x)  # 加入 Dropout
    outputs = keras.layers.Dense(action_size, activation='linear')(x)
    
    model = keras.Model(inputs, outputs)
    optimizer = keras.optimizers.Adam(learning_rate=learning_rate, clipnorm=1.0)  # clipnorm梯度裁剪
...
if load_pre_train:
    try:
        ...
        target_model.set_weights(model.get_weights())  # 初始時同步權重
